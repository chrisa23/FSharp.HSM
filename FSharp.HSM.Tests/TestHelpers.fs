...
let attachShow (hsm: IStateMachine<_, _, _>) =
    hsm.StateChanged.Add(fun state -> printfn "%A" state)

let attachHistory (hsm: IStateMachine<'state, _, _>) (store: ResizeArray<'state>) =
    hsm.StateChanged.Add(fun state -> store.Add state)

let attachOutputHistory (hsm: IStateMachine<_, _, 'output>) (store: ResizeArray<'output>) =
